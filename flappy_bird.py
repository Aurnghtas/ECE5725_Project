...
def GPIO17_callback(channel):
    global new_round_button_pressed
    new_round_button_pressed = True
    new_round_button_pressed_event_Event = pygame.event.Event(new_round_button_pressed_event)
    pygame.event.post(new_round_button_pressed_event_Event)
...
def create_pipe():
    random_y = random.randint(25, height-pipe_gap-50)
...
def detect_collision():
    ...
    if bird_rect.top <= -40 or bird_rect.bottom >= 260:
        return True
...
def draw_leaderboard():
    ...
    leaderboard_pos = (width/2+5, height-base_rect1.height/2+5)
...
def draw_leaderboard_data():
    ...
    data_rect = data_surface.get_rect(topleft=(0,40))
    screen.blit(data_surface, data_rect)

    data_surface = data_font.render("Score", True, WHITE)
    data_rect = data_surface.get_rect(topleft=(160, 40))
    screen.blit(data_surface, data_rect)
    ...
    for entry in data:
        ...
        data_rect = data_surface.get_rect(topleft=(0, 40+index*20))
        screen.blit(data_surface, data_rect)

        data_surface = data_font.render(str(entry["score"]), True, WHITE)
        data_rect = data_surface.get_rect(topleft=(160, 40+index*20))
        screen.blit(data_surface, data_rect)
    ...
    # back button
    back_font = pygame.font.Font(None, 25)
    ...
    back_box_rect.topleft = ((width-total_width)/2, 210)
    back_rect.center = back_box_rect.center
    restart_box_rect.topleft = ((width-total_width)/2+back_box_rect.width+back_restart_spacing, 210)
...
def draw_game_over():
    ...
    restart = "Press Button 17 to Play Again"
    restart_pos = (width/2, height/2+10)
    restart_surface = restart_font.render(restart, True, WHITE)
    ...
    for row in keyboard_layout:
        ...
        for character in row:
            if character == "Space":
                ...
                key_width = KEY_WIDTH
            key_rect = pygame.Rect(key_x, key_y, key_width+KEY_PADDING, KEY_HEIGHT+KEY_PADDING)
...
BLACK = 0, 0, 0
WHITE = 255, 255, 255
size = width, height = 320, 240 # screen size of the piTFT
...
base = pygame.transform.scale(base, (width, int(base_scaled_height)))
...
bird_rect = bird.get_rect(center=(width/3, (height-base_rect1.height)/2))
bird_speed = 0
bird_spawn_time = 0
...
pipe_image = pygame.transform.scale(pipe_image, (int(pipe_image_scaled_width), int(pipe_image_scaled_height)))
...
# quit button to force quit the game
quit_button_pressed = False
new_round_button_pressed = False
new_round_button_pressed_event = pygame.USEREVENT+1

# add support for accelerometer 
accelerometer_event = pygame.USEREVENT+2
time_inverval = 100
last_post_time = 0
y = 0
running = True
thread = threading.Thread(target=read_accelerometer)
thread.start()
...
while True:
    ...
    if y < -10:
        accelerometer_event_EVENT = pygame.event.Event(accelerometer_event)
        pygame.event.post(accelerometer_event_EVENT)
    ...
    for event in pygame.event.get():
        ...
        if welcome:
            ...
            if event.type == pygame.MOUSEBUTTONUP and not game_over:
                pipe_gap = 75
                bird_speed = -17*gravity
                wing_audio.play()
            
            elif event.type == accelerometer_event and not game_over:
                pipe_gap = 100
                bird_speed = -17*gravity
                wing_audio.play()

            elif new_round_button_pressed and game_over:
                game_over = False
                new_round_button_pressed = False
                score = 0
                bird_index = 0
    ...
    if welcome:
        ...
        if not game_over:
            ...
            if score < easy:
                ...
                if pipe_spawn_time > 1250:  # spawn a pipe every 1.25 seconds
                    pipe_list.append(create_pipe())
                    pipe_spawn_time = 0
            else:
                if pipe_spawn_time > 1000:  # spawn a pipe every 1 second
                    pipe_list.append(create_pipe())
                    pipe_spawn_time = 0
